...
const Modal = ({ dialogRef }: ModalInterface) => (
  ...
  >
    ...
    <p>
      Liebes Team von The Key – Technology. Eure Hausaufgabe hat mir viel Freude
      bereitet. Es gibt noch ein paar offene Punkte, bei denen ich ein bisschen
      Unterstützung gebraucht hätte:
    </p>
    <ul className='list-disc m-4'>
      <ListItem>
        <strong>Pagination:</strong> Normalerweise würde es statt diesem Button
        hier auch eine Infinite-Scroll-Funktion mit einem{' '}
        <code>Intersection-Observer</code> geben. Theoretisch war mir der Ansatz
        klar, dass ich ein <code>Relay-Fragment</code> mit{' '}
        <code>usePaginationFragment</code> verwenden konnte, um mit der{' '}
        <code>loadNext</code>-Funktion die neuen Daten zu fetchen. Tja, ich habe
    ...
    <p>
      <strong>Mein Learning:</strong> Initiale Setups von neuen Projekten haben
      ihre eigenen Herausforderungen. Sobald ein Projekt gut aufgesetzt ist,
      gehen die nächsten Schritte meist reibungsloser.
    </p>
  </dialog>
)
